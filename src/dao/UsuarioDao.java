...
public class UsuarioDao {
	...
	public UsuarioBean pesquisar(UsuarioBean usuarioBean) throws Exception {
		...
		this.connection = conexao.getConnection();
		UsuarioBean bean = new UsuarioBean();
		String sql = ("SELECT us.idusuario,us.nmusuario,us.nmlogin,us.nmsenha,us.nmsexo,s.idsetor,us.idsetor,us.datanascimento FROM usuario us INNER JOIN setor s ON ( us.idSetor = s.idSetor ) WHERE idUsuario =" + usuarioBean.getIdusuario());
		...
		if (rs.next()) {
			...
			String data = rs.getString("datanascimento");
			String novaData = Data.converteData1(data);
	...
	public void altera(UsuarioBean usuarioBean) throws Exception {
		...
		sql.append("datanascimento = '" + usuarioBean.getData() + "'");
